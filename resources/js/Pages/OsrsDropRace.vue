<template>
    ...
    <guide-layout
        current-path="/osrs-drop-race"
        :title="$t('landing.drop_race.title')"
        :description="$t('landing.drop_race.lead')"
        :sections="sections"
    ...
    >
        ...
        <section id="how-it-works">
            ...
            <div class="grid md:grid-cols-2 gap-x-8">
                <div>
                    <h3 :class="prose.h3">{{ $t('landing.drop_race.host_title') }}</h3>
                    <p :class="prose.p">{{ $t('landing.drop_race.host_subtitle') }}</p>
                    ...
                    <guide-screenshot class="mt-4" :alt="$t('landing.drop_race.screenshot_editor_alt')" />
                </div>

                <div>
                    <h3 :class="prose.h3">{{ $t('landing.drop_race.player_title') }}</h3>
                    <p :class="prose.p">{{ $t('landing.drop_race.player_subtitle') }}</p>
                    ...
                    <guide-screenshot class="mt-4" :alt="$t('landing.drop_race.screenshot_board_alt')" />
                </div>
            </div>
        </section>

        <section id="why">
            <h2 :class="prose.h2">{{ $t('landing.drop_race.why_title') }}</h2>
            <p :class="prose.p">{{ $t('landing.drop_race.why_body') }}</p>
        </section>

        <section id="tracking">
            <h2 :class="prose.h2">{{ $t('landing.drop_race.modes_title') }}</h2>
            <p :class="prose.p">{{ $t('landing.drop_race.modes_subtitle') }}</p>
...
const sections = [
    ...
    { id: 'why', label: trans('landing.drop_race.why_title') },
    { id: 'tracking', label: trans('landing.drop_race.modes_title') },
    { id: 'faq', label: faqTitle },
];

const quickFacts = [
    { label: trans('landing.drop_race.fact_metric'), value: trans('landing.drop_race.fact_metric_value') },
    { label: trans('landing.drop_race.fact_scoring'), value: trans('landing.drop_race.fact_scoring_value') },
    { label: trans('landing.drop_race.fact_setup'), value: trans('landing.drop_race.fact_setup_value') },
    { label: trans('landing.drop_race.fact_players'), value: trans('landing.drop_race.fact_players_value') },
];
</script>
