        ...
        <svg class="app-bg__field" xmlns="http://www.w3.org/2000/svg">
            <defs>
                <!-- The repeating unit. Every motif draws it around the same
                     ...
                     The opacities differ per motif on purpose: they are set
                     from the ink each tile carries per square pixel, not by
                     eye, because a diamond outline and a filled square at the
                     same opacity do not read as the same weight. Drawn to
                     taste they came out at 0.11 / 0.07 / 0.07 and the bingo
                     page looked like the pattern had failed to load.

                     Equal ink is the starting point, not the answer: matched
                     at ~0.10 the diamonds still read louder than the squares,
                     because the lattice puts twice as many shapes in the same
                     area and the eye counts edges, not coverage. Bingo sits at
                     about 0.08 for that reason. -->
                <g :id="`bg-unit-${name}`">
                    ...
                    <template v-else-if="name === 'bingo'">
                        <path :d="diamond(19)" fill="currentColor" fill-opacity="0.085" />
                        <path :d="diamond(19)" fill="none" stroke="currentColor" stroke-opacity="0.38" />
                        <path :d="diamond(7)" fill="currentColor" fill-opacity="0.17" />
                    </template>

                    <!-- A Snakes and Ladders board: cells and a ladder. The
                         ladder stays wholly inside the tile — a shape that
