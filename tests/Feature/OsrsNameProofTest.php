 ...
 * else, and two accounts on the dev database really did both hold "Main Sample".
...
class OsrsNameProofTest extends TestCase
{
    ...
    {
        $this->api()
            ->postJson('/api/plugin/v1/identity', ['rsn' => 'iron_sample'])
            ->assertOk()
    ...
    {
        $this->api()->postJson('/api/plugin/v1/identity', ['rsn' => 'Iron Sample'])->assertOk();
        ...
        $this->assertSame('Iron Sample', $this->player->fresh()->osrs_username);
    }
    ...
    {
        Http::fake(['api.wiseoldman.net/v2/players/*' => Http::response(['displayName' => 'Iron Sample'])]);
        $this->player->forceFill(['osrs_proven_at' => now(), 'osrs_proven_via' => 'runelite'])->save();
