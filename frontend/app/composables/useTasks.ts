 ...
 */
export async function useTasks(search?: MaybeRef<string | undefined> | ComputedRef<string | undefined>) {
  const vars = computed(() => ({ search: toValue(search) || undefined }))
  const { data, pending, error, refresh } = await useGql<{ tasks: TaskEntity[] }>(TASKS_QUERY, vars)

  async function createTask(input: CreateTaskInput): Promise<TaskEntity> {
    const result = await useGqlMutation<{ createTask: TaskEntity }>(CREATE_TASK_MUTATION, { input })
