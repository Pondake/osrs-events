  ...
  <nuxt-layout :title="$t('admin.users_title')" :description="$t('admin.users_subtitle')">
    <u-page-body>
      <u-container>
        ...
        <!-- Table -->
        <u-table v-else-if="users.length > 0" :data="users" :columns="tableColumns">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="font-medium truncate">
                  {{ row.original.nickname ?? row.original.discordUsername }}
                </p>
          ...
          <template #actions-cell="{ row }">
            <div class="flex justify-end gap-1">
              ...
              <u-button
                v-if="
                  row.original.id !== authStore.user?.id &&
                  !row.original.userRoles?.some((ur: any) => ur.role.name === 'ADMIN')
                "
                size="xs"
                variant="ghost"
                color="error"
          ...
          <u-alert
            ...
            :description="
              $t('admin.delete_user_confirm', {
                name: userToDelete.nickname ?? userToDelete.discordUsername,
              })
            "
          />
        </div>
...
const {
  ...
  assignRole,
  removeRole,
} = await useUsers(computed(() => debouncedSearch.value || undefined));
...
const availableRoles = computed(() => [
  ...
  {
    name: 'TEAM_MANAGER',
    label: t('admin.role_label_team_manager'),
    description: t('admin.role_desc_team_manager'),
  },
]);
...
async function toggleRole(user: UserEntity, role: string, value: boolean) {
  ...
  try {
    const updated = value ? await assignRole(user.id, role) : await removeRole(user.id, role);
  ...
  () => [
    {
      ...
      label: t('admin.perm_label_can_create_boards'),
      description: t('admin.perm_desc_can_create_boards'),
    },
    {
      key: 'CAN_CREATE_TILES',
      label: t('admin.perm_label_can_create_tiles'),
      description: t('admin.perm_desc_can_create_tiles'),
    },
  ],
);
...
async function togglePermission(userId: string, key: string, value: boolean) {
  ...
  try {
    if (value) {
      ...
      userPermissionsMap.value[userId] = (userPermissionsMap.value[userId] ?? []).filter(
        k => k !== key,
      );
